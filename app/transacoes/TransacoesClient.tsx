...
export default function TransacoesClient({ initialTransactions, initialCategories }: Props) {
  ...
  const allCategories = useMemo(() => {
    if (filterType === 'income') return initialCategories.filter((c) => c.type === 'income').map((c) => c.name)
    if (filterType === 'expense') return initialCategories.filter((c) => c.type === 'expense').map((c) => c.name)
    return initialCategories.map((c) => c.name)
  }, [filterType, initialCategories])
      ...
      {filtered.length > 0 && (
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-4 text-sm">
            <span className="text-success font-medium">+ {formatCurrency(filteredIncome)}</span>
            <span className="text-danger font-medium">- {formatCurrency(filteredExpenses)}</span>
            <button
              onClick={() => setSortOrder((s) => (s === 'desc' ? 'asc' : 'desc'))}
              className="text-text-muted hover:text-text-primary text-xs border border-border rounded-lg px-3 py-1.5 hover:bg-surface-2 transition-colors"
            >
